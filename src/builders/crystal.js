...
util.assign(Builder.prototype, {
  ...
  _outputPathname: function() {
    return this.filename.replace(/\.peg$/, '.cr');
  },
  ...
  syntaxNodeClass_: function() {
    ...
    this._indent(function(builder) {
      builder._line('include Enumerable(String)');
      
      builder.attributes_(['text', 'offset', 'elements']);
      builder.method_('initialize', ['@text : String', '@offset : Int32', '@elements = [] of TreeNode | Int32'], function(builder) {
      });
      builder.method_('each', [], function(builder) {
        builder._line('@elements.each do |element|');
        builder._line(' yield element')
        builder._line('end');
      });
    });
    this._line('end');
  ...
  grammarModule_: function(actions, block, context) {
    this.assign_('ParseError', 'Exception.new("Parse error")');
    this._newline();
    this.assign_(this.nullNode_(), 'TreeNode.new("", 0)');
    this._newline();
  ...
  parserClass_: function(root) {
    ...
    this._indent(function(builder) {
      ...
      builder.instance_attribute_('input_size', 'Int32');
      builder._methodSeparator = '\n';

      builder.method_('initialize', ['@input : String', '@actions : String', '@types : String'], function(builder) {
        builder.attribute_('input_size', '@input.size');
        builder.attribute_('offset', '0');
        builder.attribute_('cache', 'Hash(Symbol, Hash(Int32, Array(Search::TreeNode | Int32))).new{|h,k| h[k] = {} of Int32 => Array(Search::TreeNode | Int32)}');
        builder.attribute_('failure', '0');
        builder.attribute_('expected', '[] of String');
      });

      builder.method_('parse', [], function(builder) {
        builder.jump_('tree', root);
        builder.if_('tree != ' + builder.nullNode_() + ' && @offset == @input_size', function(builder) {
        ...
        builder._line('raise Parser.format_error(@input, @failure, @expected)');
      });

      builder.method_('self.format_error', ['input : String', 'offset : Int32', 'expected : Array(String)'], function(builder) {
        ...
        builder._line('end');
        builder._line('message, line = "Line #{line_no}: expected #{expected.join(", ")}\\n", lines[line_no - 1]');
  ...
  exports_: function() {
    this._line('def self.parse(input : String, options = {} of String => String)');
    this._indent(function(builder) {
  ...
  cache_: function(name, block, context) {
    var temp      = this.localVars_({address: this.nullNode_(), index: '@offset'}),
        ...
        offset    = temp.index,
        cacheMap  = '@cache[:' + name + ']',
        cacheAddr = cacheMap + '[' + offset + ']'; //TODO: check if this is the way

    this.assign_('cached', cacheAddr + '?');

    this.if_('cached', function(builder) {
      builder._line('@offset = cached[1].as(Int32) || 0');
  ...
  attributes_: function(names) {
    ...
    for (var i = 0, n = names.length; i < n; i++) {        
        keys.push(':' + names[i].toLowerCase());
    }
    this._line('getter ' + keys.join(', '));
    this._methodSeparator = '\n';
  },
  ...
  instance_attribute_: function(name, type) {
      this._line('@' + name + ' : ' + type);
  },
  ...
  failure_: function(address, expected) {
    ...
    this.if_('@offset > @failure', function(builder) {
      builder.assign_('@failure', '@offset');
      builder.assign_('@expected', '[] of String');
    });
    this.if_('@offset == @failure', function(builder) {
